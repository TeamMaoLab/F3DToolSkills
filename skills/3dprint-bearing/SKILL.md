...
description: >-
  F3DToolkit 应用：为 3D 打印而生的孔内成型球轴承——在 Fusion 360 任意位置/姿态的
  ...
  即使用户只说"给这个孔做个轴承"也要触发。依赖底座 f3dtoolkit 的 exec 端点（:9099）。
---

# 3D 打印轴承 · 孔内成型（F3DToolkit 应用）
...
1. `curl -s --max-time 5 http://127.0.0.1:9099/ping` —— 不通则按底座 f3dtoolkit 的「安装规范」装/启动 add-in
2. Fusion 打开目标文档
