...
name: stl-export
description: >-
  F3DToolkit 应用：导出当前 Fusion 文档全部可见 occurrence 的 STL（注入脚本形态，
  幂等、lightbulb 祖先链处理、exists() 双校验）。当用户要导出 STL、导出装配体、
  导出零件打印、批量导出可见组件时使用。依赖底座 f3dtoolkit 的 exec 端点。
---

# stl-export（F3DToolkit 应用 #1）
...
- 输出到系统临时目录 `f3dtoolkit_exports/`，每个 occurrence 一个 STL
  （`fullPathName` 命名，去重 `:`/`/`）；
